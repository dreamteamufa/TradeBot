const appversion = "Level07_DualStrategy_CandleSuite_v3.2_MG_EarlyClose_Stable";
...
function setStakeByKeysForCurrentStep(doneCb) {
    try {
        ...
        window.totalWager = (window.totalWager || 0) + getNextStake();
...
let expiryTime = 1; // Для старой стратегии; для новой — 2 (см. ниже)
// === Noise filter settings ===
const atrPeriod = 14;                 // ATR period (bars)
const minATRTicks = 4;                // Minimum ATR in ticks to allow trading
const stdPeriod = 20;                 // StdDev period over close-to-close returns
const minSTDTicks = 1;              // Minimum StdDev (in ticks), было 2
const minDistanceEMATicks = 0.5;        // Require |price-EMA| >= this many ticks
...
const emaFastPeriod = 20; // EMA быстрая для новой стратегии
const emaSlowPeriod = 50; // EMA медленная для новой стратегии
const rsiNewPeriod = 14; // RSI для новой стратегии
const expiryNew = 2; // Экспирация для новой стратегии (2 минуты)
const tickSize = 0.00001; // Минимальный тик для касания (адаптировать под пару)
const payoutRate = 0.92; // Выплата для бинарных опционов (92%)
...
let botStartBalance = null;
let currentBalance = 0; // Инициализируем начальный баланс 0, будет обновляться через updateBalance
let simulatedSignals = []; // Массив: {id: num, time: timestamp, entryPrice: num, direction: str, stake: num, expiry: expiryNew * 60000}
let simWinCount = 0;
let simLossCount = 0;
let simTotalPL = 0;
let mgStateSim = { // Копия реального mgState для симуляции Martingale
    level: 0, // Начальный уровень
    step: 0,
    currentMaxStep: 0,
    cycleStartBalance: botStartBalance || currentBalance // Используем реальный баланс как стартовый
};
...
function __ensureCycleStartBalance() {
    try {
        if (typeof currentBalance !== 'number' || isNaN(currentBalance)) {
            currentBalance = botStartBalance || 0; // Запасное значение, если не инициализировано
            console.warn('[MG] currentBalance not set, using default:', currentBalance);
        }
        if (mgState.cycleStartBalance == null) {
        ...
        if (mgStateSim.cycleStartBalance == null) {
            mgStateSim.cycleStartBalance = mgState.cycleStartBalance;
            console.log(`[MG SIM] Cycle start balance set: ${mgStateSim.cycleStartBalance.toFixed(2)}`);
        }
    } catch (e) {
        console.error("Error in __ensureCycleStartBalance:", e);
    }
}
...
function __updateMartingaleAfterResult(result /* 'profit' | 'loss' */) {
    ...
    if (result === 'profit') {
        mgState.step = 0;
        window.totalWins = (window.totalWins || 0) + 1;
    } else if (result === 'loss') {
        mgState.step = Math.min(mgState.step + 1, arr.length - 1);
        window.totalLosses = (window.totalLosses || 0) + 1;
    }

    window.totalTrades = (window.totalTrades || 0) + 1;
    mgState.currentMaxStep = Math.max(mgState.currentMaxStep, mgState.step);
    console.log(`[MG] After ${result}: step=${mgState.step}, maxStep=${mgState.currentMaxStep}`);
}
...
function checkTradingConditions() {  
  try {  
    ...
    // Ограничение: не чаще 1 сигнала в минуту  
    const lastSignal = signalHistory.length > 0 ? signalHistory[signalHistory.length - 1].timestamp : 0;  
    if (Date.now() - lastSignal < 60000) {  
      return;  
    }  
    let direction = null;  
    if (currentPrice > currentEMA && prevRSI < 30 && currentRSI > 30) {  
      direction = 'UP';  
    } else if (currentPrice < currentEMA && prevRSI > 70 && currentRSI < 70) {  
      direction = 'DOWN';  
    }  
    if (direction) {  
      totalSignals++;  
      const signal = {  
        ...
        ema: currentEMA,  
        rsi: currentRSI,  
        filterResults: { atr: false, std: false, dist: false, range: false },  
        outcome: 'blocked'  
      };  
      console.log(`[OLD] Potential signal: ${direction} at price ${currentPrice.toFixed(5)}, EMA ${currentEMA.toFixed(5)}, RSI ${currentRSI.toFixed(2)}`);  
      // Noise filters  
      try {  
        const __f = evaluateFilters(currentPrice, currentEMA);  
        window.__L05_lastFilters = __f;  
        signal.filterResults = {  
          atr: __f.reasons.passATR,  
          std: __f.reasons.passSTD,  
          dist: __f.reasons.passDist,  
          range: __f.reasons.passRange  
        };  
        console.log(`[OLD] Filter values: ATR=${__f.atrTicks.toFixed(1)}t (pass=${__f.reasons.passATR}), STD=${__f.stdTicks.toFixed(1)}t (pass=${__f.reasons.passSTD}), Dist=${__f.distTicks.toFixed(1)}t (pass=${__f.reasons.passDist}), Range=${__f.rangeTicks.toFixed(1)}t (pass=${__f.reasons.passRange})`);  
        if (__f.pass) {  
          signal.outcome = 'passed';  
          passedSignals++;  
          console.log(`[OLD] Signal passed filters: ${direction}, Filters: ATR=${signal.filterResults.atr}, STD=${signal.filterResults.std}, Dist=${signal.filterResults.dist}, Range=${signal.filterResults.range}`);  
          executeTrade(direction);  
        } else {  
          console.log(`[OLD] Signal blocked by filters: ${direction}, Filters: ATR=${signal.filterResults.atr}, STD=${signal.filterResults.std}, Dist=${signal.filterResults.dist}, Range=${signal.filterResults.range}`);  
        }  
      } catch(e) {  
    ...
    // Новая стратегия (параллельно старой)  
    const ema20 = calculateEMA(candlePrices.map(c => c.close), emaFastPeriod);  
    const ema50 = calculateEMA(candlePrices.map(c => c.close), emaSlowPeriod);  
    const rsi14 = calculateRSI(candlePrices.map(c => c.close), rsiNewPeriod);  
    const lastCandle = candlePrices[candlePrices.length - 1];  
    let newSignal = null;  
    let patternInfo = { isPattern: false, type: 'none' };  

    if (ema20 > ema50) { // Up-тренд для CALL  
      const touched = (lastCandle.low <= Math.min(ema20, ema50) + tickSize); // Откат вниз, касание EMA  
      patternInfo = detectBullishPattern(candlePrices);  
      if (touched && rsi14 > 50 && patternInfo.isPattern) {  
        newSignal = 'UP';  
      }  
    } else if (ema20 < ema50) { // Down-тренд для PUT  
      const touched = (lastCandle.high >= Math.max(ema20, ema50) - tickSize); // Откат вверх, касание EMA  
      patternInfo = detectBearishPattern(candlePrices);  
      if (touched && rsi14 < 50 && patternInfo.isPattern) {  
        newSignal = 'DOWN';  
      }  
    }  
    ...
    if (newSignal) {  
      console.log(`[NEW] Potential signal: ${newSignal} at price ${currentPrice.toFixed(5)}, EMA20 ${ema20.toFixed(5)}, EMA50 ${ema50.toFixed(5)}, RSI14 ${rsi14.toFixed(2)}, Pattern: ${patternInfo.type}`);  
      totalSignals++; // Общий счётчик  

      // Применяем те же фильтры шума  
      try {  
        const __f_new = evaluateFilters(currentPrice, ema20); // Dist к EMA20  
        console.log(`[NEW] Filter values: ATR=${__f_new.atrTicks.toFixed(1)}t (pass=${__f_new.reasons.passATR}), STD=${__f_new.stdTicks.toFixed(1)}t (pass=${__f_new.reasons.passSTD}), Dist=${__f_new.distTicks.toFixed(1)}t (pass=${__f_new.reasons.passDist}), Range=${__f_new.rangeTicks.toFixed(1)}t (pass=${__f_new.reasons.passRange})`);  
        if (__f_new.pass) {  
          passedSignals++;  
          console.log(`[NEW] Signal passed filters: ${newSignal}, Filters: ATR=${__f_new.reasons.passATR}, STD=${__f_new.reasons.passSTD}, Dist=${__f_new.reasons.passDist}, Range=${__f_new.reasons.passRange}`);  
          // Симуляция: Записываем сигнал с текущей ставкой из Martingale симуляции  
          const signalId = simulatedSignals.length + 1;  
          const simStake = getNextStakeSim(); // Копия реальной ставки  
          ...
          console.log(`[NEW SIM] Recorded signal ID: ${signalId}, Direction: ${newSignal}, Entry: ${currentPrice.toFixed(5)}, Stake: ${simStake}, Expiry in ${expiryNew} min`);  
        } else {  
          console.log(`[NEW] Signal blocked by filters: ${newSignal}, Filters: ATR=${__f_new.reasons.passATR}, STD=${__f_new.reasons.passSTD}, Dist=${__f_new.reasons.passDist}, Range=${__f_new.reasons.passRange}`);  
        }  
      } catch(e) {  
        console.error("Filter error for new strategy:", e);  
      }  
    }  
  } catch (e) {  
...
function mainLoop() {
    ...
    try {
        ...
        // Проверка и обновление симуляций (каждую секунду)
        const now = Date.now();
        simulatedSignals = simulatedSignals.filter(sig => {
            if (now - sig.time >= expiryNew * 60000) { // Прошло время экспирации
                const closePrice = getCurrentPriceLegacy(); // Используем текущую цену как close
                let isProfit = false;
                if (sig.direction === 'UP') {
                    isProfit = closePrice > sig.entryPrice;
                } else if (sig.direction === 'DOWN') {
                    isProfit = closePrice < sig.entryPrice;
                }
                const pl = isProfit ? (sig.stake * payoutRate) : -sig.stake;
                simTotalPL += pl;
                if (isProfit) {
                    simWinCount++;
                    __updateMartingaleAfterResultSim('profit');
                } else {
                    simLossCount++;
                    __updateMartingaleAfterResultSim('loss');
                }
                console.log(`[NEW SIM] Signal ID: ${sig.id}, Direction: ${sig.direction}, Entry: ${sig.entryPrice.toFixed(5)}, Close: ${closePrice.toFixed(5)}, Result: ${isProfit ? 'profit' : 'loss'}, P&L: ${pl.toFixed(2)} USD`);
                const totalSimTrades = simWinCount + simLossCount;
                const simHitRate = totalSimTrades > 0 ? (simWinCount / totalSimTrades * 100).toFixed(1) : 0;
                console.log(`[NEW SIM] Stats: Wins: ${simWinCount}, Losses: ${simLossCount}, Hit Rate: ${simHitRate}%, Total P&L: ${simTotalPL.toFixed(2)} USD`);
                return false; // Удаляем обработанный
            }
            return true;
        });
    } catch (e) {
...
function updateBalance() {
    try {
        // Реализация получения баланса из DOM (адаптировать селекторы под платформу)
        function getCurrentBalanceFromPlatform() {
            const selectors = [
                '.balance-value, .js-balance-value, .account-balance',
                '.top-up-balance, .user-balance, .balance-label'
            ];
            for (const sel of selectors) {
                const node = document.querySelector(sel);
                if (node) {
        ...
        const newBalance = getCurrentBalanceFromPlatform();
        if (typeof newBalance === 'number' && isFinite(newBalance)) {
            currentBalance = newBalance;
        } else if (typeof window.currentBalance !== 'undefined' && isFinite(window.currentBalance)) {
            currentBalance = window.currentBalance;
        }
        if (currentBalance !== null && (botStartBalance === null || mgState.cycleStartBalance === null)) {
            __ensureCycleStartBalance();
        }
    } catch (e) {
...
function updateUI(currentTime) {
    try {
        ...
        if (tableBody && Array.isArray(cycleHistory) && cycleHistory.length > 0) {
            let html = '';
            const recent = cycleHistory.slice(-8).reverse(); // Last 8, newest first
            recent.forEach((c, i) => {
                const cycleNum = cycleHistory.length - recent.length + i + 1;
        ...
        // Последняя сделка (сохранил, если нужно)
        if (lastTradeResult) {
            // ... (код для lastTradeResult сохранён, если актуален)
        }
        
        // Update Cycle History in UI (теперь таблица, так что вызов не нужен, но оставил для совместимости)
...
// === Полная версия IIFE для window.updateUI с добавлением индикаторов Sim P&L и Sim Hit Rate ===
(function(){
  ...
  window.updateUI = function(){
    ...
    try{
      ...
      if (document.getElementById('ui-sim-pl')) document.getElementById('ui-sim-pl').textContent = Number(simTotalPL || 0).toFixed(2);
      const totalSimTrades = simWinCount + simLossCount;
      const simHitRate = totalSimTrades > 0 ? (simWinCount / totalSimTrades * 100).toFixed(1) : 0;
      if (document.getElementById('ui-sim-hit-rate')) document.getElementById('ui-sim-hit-rate').textContent = simHitRate + '%';
    }catch(e){}
  };
})(); /*L05 WR*/

// === Полная версия функции __createLevel05UI с добавлением индикаторов ===
function __createLevel05UI() {
    try {
        let uiContainer = document.getElementById('level05-ui');
        if (!uiContainer) {
            uiContainer = document.createElement('div');
            uiContainer.id = 'level05-ui';
            uiContainer.style = 'position: fixed; top: 10px; right: 10px; background: rgba(0,0,0,0.7); color: #fff; padding: 10px; border-radius: 5px; z-index: 10000; font-size: 12px;'; // Увеличен z-index
            document.body.appendChild(uiContainer);
            console.log("UI container created successfully");
        } else {
            console.log("UI container already exists, updating...");
            uiContainer.innerHTML = ''; // Очищаем перед обновлением
        }
        uiContainer.innerHTML = ''; // Очищаем перед добавлением

        const appLabel = document.createElement('div');
        appLabel.textContent = appversion;
        uiContainer.appendChild(appLabel);

        const balanceLabel = document.createElement('div');
        balanceLabel.id = 'ui-balance';
        balanceLabel.textContent = 'Balance: 0.00';
        uiContainer.appendChild(balanceLabel);

        const plLabel = document.createElement('div');
        plLabel.id = 'ui-pl';
        plLabel.textContent = 'P&L: 0.00';
        uiContainer.appendChild(plLabel);

        const tradesLabel = document.createElement('div');
        tradesLabel.id = 'ui-trades-count';
        tradesLabel.textContent = 'Trades: 0';
        uiContainer.appendChild(tradesLabel);

        const winsLabel = document.createElement('div');
        winsLabel.id = 'ui-wins-count';
        winsLabel.textContent = 'Wins: 0';
        uiContainer.appendChild(winsLabel);

        const lossesLabel = document.createElement('div');
        lossesLabel.id = 'ui-losses-count';
        lossesLabel.textContent = 'Losses: 0';
        uiContainer.appendChild(lossesLabel);

        const hitRateLabel = document.createElement('div');
        hitRateLabel.id = 'ui-hit-rate';
        hitRateLabel.textContent = 'Hit Rate: 0.0%';
        uiContainer.appendChild(hitRateLabel);

        const wagerLabel = document.createElement('div');
        wagerLabel.id = 'ui-wager';
        wagerLabel.textContent = 'Wager: 0.00';
        uiContainer.appendChild(wagerLabel);

        // Новые индикаторы для симуляции
        const simPLLabel = document.createElement('div');
        simPLLabel.id = 'ui-sim-pl';
        simPLLabel.textContent = '0.00'; // Sim P&L
        uiContainer.appendChild(document.createTextNode('Sim P&L: '));
        uiContainer.appendChild(simPLLabel);

        const simHitLabel = document.createElement('div');
        simHitLabel.id = 'ui-sim-hit-rate';
        simHitLabel.textContent = '0%'; // Sim Hit Rate
        uiContainer.appendChild(document.createTextNode('Sim Hit Rate: '));
        uiContainer.appendChild(simHitLabel);
    } catch(e) {
        console.error('__createLevel05UI error:', e);
    }
}
...
let cycleHistory = [];
// Детекция bullish паттернов (для CALL: длинный нижний хвост — pinbar или engulfing)
function detectBullishPattern(candles) {
    if (candles.length < 2) return { isPattern: false, type: 'none' };
    ...
    // Bullish Pinbar: Длинный нижний wick (>=2*body), маленький upper wick (<body/2), body в верхней половине
    const isPinBull = (lowerWick >= 2 * body) && (upperWick < body / 2) && (Math.min(last.open, last.close) > last.low + (range / 2));

    // Bullish Engulfing: Предыдущая down (красная), текущая up (зелёная), тело текущей поглощает предыдущую
    const isEngBull = (prev.close < prev.open) && (last.close > last.open) && (last.open <= prev.close) && (last.close >= prev.open);

    return { isPattern: isPinBull || isEngBull, type: isPinBull ? 'pinbar' : (isEngBull ? 'engulfing' : 'none') };
}

// Детекция bearish паттернов (для PUT: длинный верхний хвост — pinbar или engulfing)
function detectBearishPattern(candles) {
    if (candles.length < 2) return { isPattern: false, type: 'none' };
    const last = candles[candles.length - 1];
    const prev = candles[candles.length - 2];
    const body = Math.abs(last.close - last.open);
    ...
    // Bearish Pinbar: Длинный верхний wick (>=2*body), маленький lower wick (<body/2), body в нижней половине
    const isPinBear = (upperWick >= 2 * body) && (lowerWick < body / 2) && (Math.max(last.open, last.close) < last.high - (range / 2));

    // Bearish Engulfing: Предыдущая up (зелёная), текущая down (красная), тело текущей поглощает предыдущую
    const isEngBear = (prev.close > prev.open) && (last.close < last.open) && (last.open >= prev.close) && (last.close <= prev.open);

    return { isPattern: isPinBear || isEngBear, type: isPinBear ? 'pinbar' : (isEngBear ? 'engulfing' : 'none') };
}
function getNextStakeSim() {
    const arr = __betLevels[mgStateSim.level] || __betLevels[0];
    const cur = arr[Math.min(mgStateSim.step, arr.length - 1)];
    return cur ? cur.value : 1; // Возвращаем value ставки
}
function __updateMartingaleAfterResultSim(result /* 'profit' | 'loss' */) {
    const arr = __betLevels[mgStateSim.level] || __betLevels[0];
    if (!arr || arr.length === 0) {
        console.warn('[MG SIM] Invalid bet array, resetting to step 0');
        mgStateSim.step = 0;
        return;
    }
    ...
    if (result === 'profit') {
        ...
        mgStateSim.step = Math.min(mgStateSim.step + 1, arr.length - 1);
    }

    mgStateSim.currentMaxStep = Math.max(mgStateSim.currentMaxStep, mgStateSim.step);
    console.log(`[MG SIM] After ${result}: step=${mgStateSim.step}, maxStep=${mgStateSim.currentMaxStep}`);
}