const appversion = "Level06_MultiStrategy_v3.1_MG_EarlyClose_MG"
      ...
      if (strat.type === 'virtual') {
        ...
        if (vState.active && !vState.resultRecorded) {
          ...
          if (timeSinceTrade > strat.expiryTime * 60000 * 2) {
            console.log(`Clearing stuck virtual trade state for ${strat.name}`);
            vState.active = false;
          }
        }
      }
...
function updateUI(currentTime) {
  try {
    ...
    // Добавлено: Обновление virtual UI
    strategies.forEach(strat => {
      if (strat.type === 'virtual') {
