...
function calculateEMA(period, candles) {
    if (candles.length < period) return null; // Недостаточно данных
    const multiplier = 2 / (period + 1);
    let ema = candles.slice(0, period).reduce((sum, c) => sum + c.close, 0) / period; // Начальный SMA
...
function calculateRSI(period = 14, candles) {
    if (candles.length < period + 1) return null; // Нужно минимум period + 1 свечей
    let gains = 0, losses = 0;
    for (let i = 1; i <= period; i++) {
        const change = candles[i].close - candles[i - 1].close;
        if (change > 0) gains += change;
        else losses -= change; // Абсолютное значение
    }
    ...
    const rs = avgGain / (avgLoss || 1); // Избежать деления на 0
    return 100 - (100 / (1 + rs));
}
