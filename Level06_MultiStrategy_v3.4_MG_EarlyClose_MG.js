const appversion = "Level06_MultiStrategy_v3.4_MG_EarlyClose_MG"
...
function updateIndicators() {
  ...
  if (currentEMA !== null) {
    ...
    strategies.find(s => s.name === 'Strategy1.0').emaValues = emaValues; // Синхронизация
  }
  ...
  if (candlePrices.length >= rsiPeriod + 1) {
    ...
    if (currentRSI !== null) {
      ...
      strategies.find(s => s.name === 'Strategy1.0').rsiValues = rsiValues; // Синхронизация
    }
  }

  // Обновление виртуальных стратегий (оставляем для совместимости)
  strategies.forEach(strat => {
    if (strat.name === 'Strategy2.0' && strat.type === 'virtual') {
      const currentEMAFast = calculateEMA(closePrices, strat.emaFastPeriod);
      const currentEMASlow = calculateEMA(closePrices, strat.emaSlowPeriod);
...
function mainLoop() {
  ...
  try {
    ...
    if (tradeState.active && !tradeState.resultRecorded) {
      const timeSinceTrade = currentTime - tradeState.startTime;
      if (timeSinceTrade > expiryTime * 60000 * 2) {
    ...
    updateIndicators(); // Убедитесь, что эта строка есть и выполняется перед updateUI
  ...
  <div style="display:flex; gap:10px; align-items:flex-start;">
    ...
    <div style="flex:1;">
      ...
      <div style="display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:6px; margin-bottom:8px;">
        ...
        <div><strong>Start Time:</strong> <span id="ui-start-time">-</span></div>
        <div><strong>Runtime:</strong> <span id="ui-runtime">-</span></div>
        <div><strong>Account:</strong> <span id="ui-account-type">${mode}</span></div>
        <div><strong>Start Balance:</strong> <span id="ui-start-balance">-</span></div>
        <div><strong>Balance:</strong> <span id="ui-balance">-</span></div>
        <div><strong>P&L:</strong> <span id="ui-pnl">-</span> (<span id="ui-pnl-percent">-</span>%)</div>
      ...
      <div style="margin-top:8px; background:#222; padding:6px; border-radius:4px; max-height:120px; overflow-y:auto;">
        <strong style="color:#ddd;">Cycle History (last 8):</strong>
        <table id="ui-cycle-history-table" style="width:100%; margin-top:4px; font-size:11px; color:#bbb; border-collapse:collapse;">
