...
// == Базовые настройки ==
const appversion = "Level05_CandleSuite_v2.9_MG_EarlyClose_MG";
...
function calculateEMA(prices, period) {
    ...
    for (let i = period; i < prices.length; i++) {
        ema.push(prices[i] * mult + ema[ema.length - 1] * (1 - mult));
    }
    return ema;
}
...
function updateIndicators(prices) {
    ema5Values.push(calculateEMA(prices, emaPeriod)[prices.length - 1]);
    rsi3Values.push(calculateRSI(prices, rsiPeriod)[prices.length - 1]);
    ema20Values.push(calculateEMA(prices, 20)[prices.length - 1]);
    ema50Values.push(calculateEMA(prices, 50)[prices.length - 1]);
    rsi14Values.push(calculateRSI(prices, 14)[prices.length - 1]);
}
...
function checkCandles() {
    ...
    if (price > 0) {
        if (candlePrices.length === 0 || now - lastCandleTime >= candleInterval) {
            if (candlePrices.length > 0) {
                // Process completed candle
                const completedCandle = candlePrices[candlePrices.length - 1];
                onCompletedCandle(completedCandle);
            }
            candlePrices.push(createNewCandle(now, price));
            lastCandleTime = getStartOfMinute(now);
        } else {
            updateCurrentCandle(candlePrices[candlePrices.length - 1], price);
        }
        // Calculate indicators
        updateIndicators(candlePrices.map(c => c.close));

        // Process strategies
        strategies.forEach(strategy => {
            const signal = strategy.generateSignal(candlePrices);
            if (signal) {
                if (!strategy.isVirtual) {
                    executeTrade(signal);
                } else {
                    simulateVirtualTrade(strategy, signal, price);
                }
            }
        });
...
function simulateVirtualTrade(strategy, direction, startPrice) {
    const stake = getNextStake();
    const expiryMs = 2 * 60000; // 2 минуты минимум
    setTimeout(() => {
        const endPrice = getCurrentPrice();
        const payout = 0.92; // 92% как в скрине
        const isWin = (direction === 'UP' && endPrice > startPrice) || (direction === 'DOWN' && endPrice < startPrice);
        const profit = isWin ? stake * payout : -stake;
        strategy.virtualBalance += profit;
        strategy.virtualProfit += profit;
        strategy.virtualTrades++;
        if (isWin) strategy.virtualWins++;
        __updateMartingaleAfterResult(isWin ? 'profit' : 'loss');
        console.log(`Virtual Trade [${strategy.name}]: ${direction}, Stake: ${stake}, Result: ${isWin ? 'WIN' : 'LOSS'}, Profit: ${profit}`);
        updateUI(Date.now());
    }, expiryMs + 2000); // +2s буфер
}
...
function checkTradeResult() {
    try {
        ...
        const result = current > tradeState.startBalance ? 'profit' : 'loss';
        const profit = current - tradeState.startBalance;
        const payoutPercent = 92; // Из скрина
        tradeHistory.push({ result, profit, direction: tradeState.direction, stake: tradeState.stake });
        __updateMartingaleAfterResult(result);
        tradeState.resultRecorded = true;
        tradeState.active = false;
        // Лог для Strategy1.0
        console.log(`Strategy1.0, Close Time: ${new Date().toLocaleString()}, Asset: ${symbolName}, Result: ${result}, Payout %: ${payoutPercent}, Stake: ${tradeState.stake}`);
    } catch (e) {
        console.error('Error checking trade result:', e);
    }
}
...
function checkTradingConditions() {
    try {
        ...
        if (direction) {
            ...
            console.log(`Potential signal: ${direction} at price ${currentPrice.toFixed(5)}, EMA ${currentEMA.toFixed(5)}, RSI ${currentRSI.toFixed(2)}`);
            ...
            try {
                ...
                if (__f.pass) {
                    ...
                    passedSignals++;
                    console.log(`Signal passed filters: ${direction}, Filters: ATR=${signal.filterResults.atr}, STD=${signal.filterResults.std}, Dist=${signal.filterResults.dist}, Range=${signal.filterResults.range}`);
                    executeTrade(direction);
                } else {
                    console.log(`Signal blocked by filters: ${direction}, Filters: ATR=${signal.filterResults.atr}, STD=${signal.filterResults.std}, Dist=${signal.filterResults.dist}, Range=${signal.filterResults.range}`);
                }
            } catch(e) {
  ...
  <div style="display:flex; gap:10px; align-items:flex-start;">
    ...
    <div style="flex:1;">
      ...
      <div style="display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:6px; margin-bottom:8px;">
        ...
        ${strategies.map(s => s.isVirtual ? `
            <div><strong>${s.name} V-Balance:</strong> <span id="ui-vbal-${s.name}">-</span></div>
            <div><strong>${s.name} V-Profit:</strong> <span id="ui-vpnl-${s.name}">-</span></div>
            <div><strong>${s.name} V-Hit:</strong> <span id="ui-vhit-${s.name}">-</span>%</div>
        ` : '').join('')}
      </div>
      <!-- Cycle History Table -->
...
function updateUI(currentTime) {
    try {
        ...
        // Индикаторы
        const lastEMA = ema5Values.length > 0 ? ema5Values[ema5Values.length - 1] : null;
        const lastRSI = rsi3Values.length > 0 ? rsi3Values[rsi3Values.length - 1] : null;
        ...
        // Для виртуальных стратегий
        strategies.forEach(s => {
            if (s.isVirtual) {
                safeUpdateElement(`ui-vbal-${s.name}`, s.virtualBalance.toFixed(2));
                safeUpdateElement(`ui-vpnl-${s.name}`, s.virtualProfit.toFixed(2));
                const vHit = s.virtualTrades > 0 ? (s.virtualWins / s.virtualTrades * 100).toFixed(1) : 0;
                safeUpdateElement(`ui-vhit-${s.name}`, vHit);
            }
        });
...
function initBot(symbol, account) {
    symbolName = symbol;
    bot = new Bot(symbol, account);
    startTime = Date.now();
    startBalance = bot.getStats().startBalance || 0;
    currentBalance = startBalance;
    tradeHistory = [];
    tradeState = { active: false };
    mgState = { level: 0, step: 0, cycleStartBalance: null, currentMaxStep: 0 };
    candlePrices = [];
    ema5Values = [];
    rsi3Values = [];
    ema20Values = [];
    ...
    lastCandleTime = 0;
    strategies = [];
    strategies.push(new Strategy('Strategy1.0', false, (candles) => {
        const lastEMA = ema5Values[ema5Values.length - 1];
        const lastRSI = rsi3Values[rsi3Values.length - 1];
        if (globalPrice > lastEMA && lastRSI < 30) return 'UP';
        if (globalPrice < lastEMA && lastRSI > 70) return 'DOWN';
        return null;
    }));
    strategies.push(new Strategy('Strategy2.0', true, (candles) => {
        ...
        if (lastEMA20 > lastEMA50 && lastRSI14 > 50 && lowerWick > body * 2 && (lastCandle.low <= lastEMA20 || lastCandle.low <= lastEMA50)) return 'UP';
        if (lastEMA20 < lastEMA50 && lastRSI14 < 50 && upperWick > body * 2 && (lastCandle.high >= lastEMA20 || lastCandle.high >= lastEMA50)) return 'DOWN';
        return null;
    }));
    bot.start();
}
