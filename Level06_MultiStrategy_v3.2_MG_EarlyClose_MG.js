const appversion = "Level06_MultiStrategy_v3.2_MG_EarlyClose_MG"
...
const minSTDTicks = 1.5;              // Minimum StdDev (in ticks), было 2
...
let signalHistory = []; // История последних 50 сигналов
let totalSignals = 0;   // Общее количество потенциальных сигналов по EMA/RSI
let passedSignals = 0;  // Количество прошедших фильтры
...
let strategies = [
  {
    name: 'Strategy1.0',
    type: 'real',
    emaPeriod: 5, // существующий
    rsiPeriod: 3, // существующий
    expiryTime: 1, // существующий
    emaValues: emaValues, // ссылка на существующие
    rsiValues: rsiValues,
    signalHistory: signalHistory // существующий
  },
  {
    name: 'Strategy2.0',
    type: 'virtual',
    ...
    emaSlowPeriod: 50,
    rsiPeriod: 14,
    expiryTime: 2, // 2 минуты, как базовое (можно randomize 2-3)
    emaFastValues: [], // отдельные
    emaSlowValues: [], // отдельные
    rsiValues: [], // отдельные
    signalHistory: [], // отдельная история
    virtualBalance: 1000, // начальный виртуальный баланс, например 1000
    virtualProfit: 0,
    virtualTradeHistory: [],
    virtualWinRate: 0,
    mgState_virtual: { level: 0, step: 0, cycleStartBalance: null, currentMaxStep: 0 } // дубликат mgState
  }
  // Добавляйте новые стратегии сюда
];
let virtualTradeStates = {}; // объект для tradeState по стратегиям, e.g. { 'Strategy2.0': {active: false, ...} }
strategies.forEach(strat => {
  if (strat.type === 'virtual') {
    virtualTradeStates[strat.name] = { active: false, direction: null, startTime: 0, startBalance: 0, startPrice: 0, resultRecorded: false, checkAttempts: 0, stake: 0 };
  }
});
...
function __updateMartingaleAfterResult(result /* 'profit' | 'loss' */) {
    ...
    if (result === 'profit') {
        ...
        if (mgState.step < arr.length - 1) {
            mgState.step++;
            if (mgState.step > mgState.currentMaxStep) mgState.currentMaxStep = mgState.step;
            ...
            // Новое: Если убыток на последнем шаге, досрочно закрыть цикл как 'loss', перейти к следующему level
            console.log(`[MG] Loss on last step (${mgState.step}): Early cycle close as loss`);
            __checkCycleLimitsAndMaybeSwitch('loss', true); // true для игнора лимитов
        }
    }
}
// Check cycle limits and switch levels if needed
function __checkCycleLimitsAndMaybeSwitch(hitType, ignoreLimits = false) {
    if (mgState.cycleStartBalance == null || typeof currentBalance !== 'number') return;
    const pnl = currentBalance - mgState.cycleStartBalance;
    const winLimit = __winLimits[mgState.level] || 50;
    const lossLimit = __lossLimits[mgState.level] || -50;
    const shouldClose = ignoreLimits || (pnl >= winLimit || pnl <= lossLimit);
    if (!shouldClose) return;

    // Close cycle
    const actualHit = (ignoreLimits && hitType === 'loss') ? 'loss' : (pnl >= winLimit ? 'win' : 'loss');
    console.log(`[MG] Cycle closed: ${actualHit.toUpperCase()} (pnl: ${pnl.toFixed(2)}), maxStep: ${mgState.currentMaxStep}`);
    cycleHistory.push({
        level: mgState.level,
    ...
    // Compute next level
    const nextLevel = __computeNextLevelAfterCycle(actualHit);
    mgState.level = nextLevel;
    mgState.step = 0;
    mgState.currentMaxStep = 0;
    mgState.cycleStartBalance = currentBalance;
    console.log(`[MG] Switched to level ${mgState.level} (betArray${mgState.level + 1}), cycle start balance reset: ${mgState.cycleStartBalance.toFixed(2)}`);
}
function getNextStake_virtual(strat) {
  const arr = __betLevels[strat.mgState_virtual.level] || __betLevels[0];
  const cur = arr[Math.min(strat.mgState_virtual.step, arr.length - 1)];
  return cur && typeof cur.value === 'number' ? cur.value : __betLevels[0][0].value;
}
function __updateMartingaleAfterResult_virtual(strat, result) {
  const arr = __betLevels[strat.mgState_virtual.level] || __betLevels[0];
  if (!arr || arr.length === 0) {
    console.warn(`[MG Virtual ${strat.name}] Invalid bet array, resetting to step 0`);
    strat.mgState_virtual.step = 0;
    return;
  }
  if (result === 'profit') {
    strat.mgState_virtual.step = 0;
    console.log(`[MG Virtual ${strat.name}] Profit: Reset to step 0 (value: ${getNextStake_virtual(strat).toFixed(2)})`);
  } else if (result === 'loss') {
    if (strat.mgState_virtual.step < arr.length - 1) {
      strat.mgState_virtual.step++;
      if (strat.mgState_virtual.step > strat.mgState_virtual.currentMaxStep) strat.mgState_virtual.currentMaxStep = strat.mgState_virtual.step;
      console.log(`[MG Virtual ${strat.name}] Loss: Move to step ${strat.mgState_virtual.step} (value: ${getNextStake_virtual(strat).toFixed(2)}), currentMaxStep updated to ${strat.mgState_virtual.currentMaxStep}`);
    } else {
      console.log(`[MG Virtual ${strat.name}] Loss on last step (${strat.mgState_virtual.step}): Early cycle close as loss`);
      __checkCycleLimitsAndMaybeSwitch_virtual(strat, 'loss', true);
    }
  }
}
function __checkCycleLimitsAndMaybeSwitch_virtual(strat, hitType, ignoreLimits = false) {
  if (strat.mgState_virtual.cycleStartBalance == null || typeof strat.virtualBalance !== 'number') return;
  const pnl = strat.virtualBalance - strat.mgState_virtual.cycleStartBalance;
  const winLimit = __winLimits[strat.mgState_virtual.level] || 50;
  const lossLimit = __lossLimits[strat.mgState_virtual.level] || -50;
  const shouldClose = ignoreLimits || (pnl >= winLimit || pnl <= lossLimit);
  if (!shouldClose) return;
  const actualHit = (ignoreLimits && hitType === 'loss') ? 'loss' : (pnl >= winLimit ? 'win' : 'loss');
  console.log(`[MG Virtual ${strat.name}] Cycle closed: ${actualHit.toUpperCase()} (pnl: ${pnl.toFixed(2)}), maxStep: ${strat.mgState_virtual.currentMaxStep}`);
  // cycleHistory виртуальный? Используем общий, но с меткой
  cycleHistory.push({ level: strat.mgState_virtual.level, maxStep: strat.mgState_virtual.currentMaxStep, pnl: pnl, hitType: actualHit, endTime: Date.now(), stratName: strat.name });
  if (cycleHistory.length > 50) cycleHistory.shift();
  const nextLevel = __computeNextLevelAfterCycle(actualHit);
  strat.mgState_virtual.level = nextLevel;
  strat.mgState_virtual.step = 0;
  strat.mgState_virtual.currentMaxStep = 0;
  strat.mgState_virtual.cycleStartBalance = strat.virtualBalance;
  console.log(`[MG Virtual ${strat.name}] Switched to level ${strat.mgState_virtual.level} (betArray${strat.mgState_virtual.level + 1}), cycle start balance reset: ${strat.mgState_virtual.cycleStartBalance.toFixed(2)}`);
}
...
function calculateEMA_Strat2(prices, period) {
  if (prices.length < period) return null;
  const k = 2 / (period + 1);
  let ema = prices.slice(0, period).reduce((a, b) => a + b, 0) / period;
  for (let i = period; i < prices.length; i++) {
...
function calculateRSI_Strat2(prices, period) {
  if (prices.length < period + 1) return null;
  let gains = 0;
  let losses = 0;
  for (let i = 1; i <= period; i++) {
...
function evaluateFilters_Strat2(currentPrice, currentEMAFast, currentEMASlow, currentRSI, lastCandle) {
  // Проверка отката и хвоста
  let passPullback = false;
  let passTail = false;
  let direction = null;

  const body = Math.abs(lastCandle.open - lastCandle.close);
  const upperTail = lastCandle.high - Math.max(lastCandle.open, lastCandle.close);
  const lowerTail = Math.min(lastCandle.open, lastCandle.close) - lastCandle.low;

  if (currentEMAFast > currentEMASlow && currentRSI > 50 && currentPrice <= currentEMAFast && lowerTail > 2 * body) {
    passPullback = true;
    passTail = true;
    direction = 'UP';
  } else if (currentEMAFast < currentEMASlow && currentRSI < 50 && currentPrice >= currentEMAFast && upperTail > 2 * body) {
    passPullback = true;
    passTail = true;
    direction = 'DOWN';
  }

  const pass = passPullback && passTail;
  console.log(`Strat2 Filters: Pullback=${passPullback}, Tail=${passTail}`);
...
function evaluateFilters(currentPrice, currentEMA) {
    ...
    let passDist = false;
    try {
        passDist = (distTicks != null) ? (distTicks >= minDistanceEMATicks) : false;
    } catch(e) {
        console.error('passDist error:', e);
    }
    const passRange = (rangeTicks != null) ? (rangeTicks > narrowRangeMaxTicks) : false;
    // Log filter values
    console.log(`Filter values: ATR=${atrTicks != null ? atrTicks.toFixed(1) : '-'}t (pass=${passATR}), STD=${stdTicks != null ? stdTicks.toFixed(1) : '-'}t (pass=${passSTD}), Dist=${distTicks != null ? distTicks.toFixed(1) : '-'}t (pass=${passDist}), Range=${rangeTicks != null ? rangeTicks.toFixed(1) : '-'}t (pass=${passRange})`);
...
function updateIndicators() {
  ...
  strategies.forEach(strat => {
    if (strat.name === 'Strategy1.0') {
      const currentEMA = calculateEMA(closePrices, strat.emaPeriod);
      if (currentEMA !== null) {
        strat.emaValues.push(currentEMA);
        if (strat.emaValues.length > 100) strat.emaValues.shift();
      }
      if (candlePrices.length >= strat.rsiPeriod + 1) {
        const currentRSI = calculateRSI(closePrices, strat.rsiPeriod);
        if (currentRSI !== null) {
          strat.rsiValues.push(currentRSI);
          if (strat.rsiValues.length > 100) strat.rsiValues.shift();
        }
      }
    } else if (strat.name === 'Strategy2.0') {
      const currentEMAFast = calculateEMA_Strat2(closePrices, strat.emaFastPeriod);
      const currentEMASlow = calculateEMA_Strat2(closePrices, strat.emaSlowPeriod);
      if (currentEMAFast !== null) {
        strat.emaFastValues.push(currentEMAFast);
        if (strat.emaFastValues.length > 100) strat.emaFastValues.shift();
      }
      if (currentEMASlow !== null) {
        strat.emaSlowValues.push(currentEMASlow);
        if (strat.emaSlowValues.length > 100) strat.emaSlowValues.shift();
      }
      if (candlePrices.length >= strat.rsiPeriod + 1) {
        const currentRSI = calculateRSI_Strat2(closePrices, strat.rsiPeriod);
        if (currentRSI !== null) {
          strat.rsiValues.push(currentRSI);
          if (strat.rsiValues.length > 100) strat.rsiValues.shift();
        }
...
function initData() {
  try {
    ...
    strategies.forEach(strat => {
      if (strat.type === 'virtual') {
        strat.virtualBalance = startBalance; // Синхронизировать с реальным стартовым
      }
    });
  } catch (e) {
...
function renderCandleChart(){
  try{
    console.log('Рендеринг графика: candlePrices.length = ' + candlePrices.length);
    ...
    console.log('Размеры холста (до корректировки): width=' + rect.width + ', height=' + rect.height);
    const dpr = window.devicePixelRatio || 1;
    const maxCanvasWidth = 1200; // Максимальная ширина в пикселях
    const maxCanvasHeight = 600; // Максимальная высота в пикселях
    const cssW = Math.min(maxCanvasWidth, Math.max(260, rect.width || 300));
    const cssH = Math.min(maxCanvasHeight, Math.max(160, rect.height || 240));
    ...
    const scroll = document.getElementById('ui-scroll');
    if (scroll){ scroll.max = String(maxOffset); if (Number(scroll.value) > maxOffset) scroll.value = String(maxOffset); }
    const zoom = document.getElementById('ui-zoom');
    if (zoom){ zoom.min = String(CHART_ZOOM_MIN); zoom.max = String(CHART_ZOOM_MAX); zoom.value = String(chartBars); }
    const tfSel = document.getElementById('ui-tf');
    if (tfSel) tfSel.value = chartTimeframe;
    ...
    console.log('Отображаемые бары: ' + view.length + ' / всего: ' + total);
    ...
    if (!view.length) {
      console.log('Нет баров для отрисовки');
      return;
    }
    ...
    console.log('График успешно отрисован с размерами: ' + cssW + 'x' + cssH);
  } catch(e){
    console.error('Ошибка renderCandleChart:', e);
  }
}
...
function executeTrade(direction) {
    ...
    try {
        ...
        setStakeByKeysForCurrentStep(() => {
            ...
            // Добавлено: Инкремент Wager после размещения ставки
            window.totalWager = (window.totalWager || 0) + tradeState.stake;
...
function checkVirtualTradeResult(strat) {
  const vState = virtualTradeStates[strat.name];
  if (!vState.active || vState.resultRecorded) return;
  try {
    vState.checkAttempts++;
    const currentTime = Date.now();
    const timeSinceTrade = currentTime - vState.startTime;
    const minTradeDuration = strat.expiryTime * 60000 * 0.8;
    if (timeSinceTrade < minTradeDuration) {
      if (vState.checkAttempts < 5) {
        setTimeout(() => checkVirtualTradeResult(strat), 5000);
      } else {
        console.log(`Virtual trade check timeout for ${strat.name}`);
        vState.active = false;
      }
      return;
    }
    // Симулируем результат: сравниваем endPrice с startPrice
    const endPrice = globalPrice; // Текущая цена как end
    const isProfit = (vState.direction === 'UP' && endPrice > vState.startPrice) || (vState.direction === 'DOWN' && endPrice < vState.startPrice);
    const payout = 1.8; // Предполагаемый payout 80%, amount = stake * 0.8 for profit, -stake for loss
    const amount = isProfit ? vState.stake * (payout - 1) : -vState.stake;
    const result = isProfit ? 'profit' : 'loss';

    recordVirtualTradeResult(strat, result, amount);
  } catch (e) {
    console.error(`Error in checkVirtualTradeResult for ${strat.name}:`, e);
    vState.active = false;
  }
}
...
function recordTradeResult(result, amount) {
  try {
    ...
    const trade = { time: Date.now(), direction: tradeState.direction, result: result, amount: Math.abs(amount), balance: currentBalance, price: tradeState.startPrice, endPrice: globalPrice };
    ...
    lastTradeResult = { result: result, amount: amount, time: trade.time };
    try {
      ...
      (function(){
        try {
          ...
          let __nextStake = (typeof getNextStake === 'function') ? getNextStake({ lastResult: result, lastAmount: Math.abs(__pnl) }) : (typeof __betLevels[0][0].value !== 'undefined' ? __betLevels[0][0].value : null);
    ...
    // Добавленный лог для Strategy1.0
    const payoutPercent = result === 'profit' ? ((amount / tradeState.stake) * 100).toFixed(2) : -100;
    console.log(`Strategy1.0 | Close Time: ${new Date(trade.time).toLocaleString()} | Asset: ${symbolName} | Result: ${result.toUpperCase()} | Payout %: ${payoutPercent} | Stake: ${tradeState.stake.toFixed(2)}`);
...
function recordVirtualTradeResult(strat, result, amount) {
  try {
    const vState = virtualTradeStates[strat.name];
    if (vState.resultRecorded) return;
    const trade = { time: Date.now(), direction: vState.direction, result, amount: Math.abs(amount), balance: strat.virtualBalance, price: vState.startPrice, endPrice: globalPrice };
    strat.virtualTradeHistory.push(trade);
    if (strat.virtualTradeHistory.length > 100) strat.virtualTradeHistory.shift();
    strat.virtualBalance += amount;
    if (result === 'profit') strat.virtualProfit += amount;
    const profitableTrades = strat.virtualTradeHistory.filter(t => t.result === 'profit').length;
    strat.virtualWinRate = strat.virtualTradeHistory.length > 0 ? (profitableTrades / strat.virtualTradeHistory.length * 100) : 0;

    __updateMartingaleAfterResult_virtual(strat, result);
    __checkCycleLimitsAndMaybeSwitch_virtual(strat);

    console.log(`Virtual CONFIRMED ${result.toUpperCase()} for ${strat.name}: $${Math.abs(amount).toFixed(2)} | Virtual Balance: ${vState.startBalance.toFixed(2)} → ${strat.virtualBalance.toFixed(2)}`);

    vState.resultRecorded = true;
    vState.active = false;
  } catch (e) {
    console.error(`Error in recordVirtualTradeResult for ${strat.name}:`, e);
  }
}

function checkTradingConditions() {
  try {
    if (candlePrices.length < Math.max(emaPeriod, strategies.reduce((max, s) => Math.max(max, s.emaFastPeriod || s.emaPeriod || 0, s.rsiPeriod || 0), 0) + 1)) return;

    strategies.forEach(strat => {
      const lastSignal = strat.signalHistory.length > 0 ? strat.signalHistory[strat.signalHistory.length - 1].timestamp : 0;
      ...
      if (strat.name === 'Strategy1.0') {
        if (tradeState.active || strat.emaValues.length < 2 || strat.rsiValues.length < 2) return;
        const currentPrice = globalPrice;
        const currentEMA = strat.emaValues[strat.emaValues.length - 1];
        const prevEMA = strat.emaValues[strat.emaValues.length - 2];
        const currentRSI = strat.rsiValues[strat.rsiValues.length - 1];
        const prevRSI = strat.rsiValues[strat.rsiValues.length - 2];

        if (currentPrice > currentEMA && prevRSI < 30 && currentRSI > 30) direction = 'UP';
        else if (currentPrice < currentEMA && prevRSI > 70 && currentRSI < 70) direction = 'DOWN';

        if (direction) {
          totalSignals++;
          const signal = { timestamp: Date.now(), direction, price: currentPrice, ema: currentEMA, rsi: currentRSI, filterResults: { atr: false, std: false, dist: false, range: false }, outcome: 'blocked' };
          console.log(`Potential signal ${strat.name}: ${direction} at price ${currentPrice.toFixed(5)}, EMA ${currentEMA.toFixed(5)}, RSI ${currentRSI.toFixed(2)}`);
          const __f = evaluateFilters(currentPrice, currentEMA);
          window.__L05_lastFilters = __f;
          signal.filterResults = { atr: __f.reasons.passATR, std: __f.reasons.passSTD, dist: __f.reasons.passDist, range: __f.reasons.passRange };
          filtersPass = __f.pass;
          if (filtersPass) {
            signal.outcome = 'passed';
            passedSignals++;
            console.log(`Signal passed filters ${strat.name}: ${direction}`);
            executeTrade(direction); // Реальная торговля
          } else {
            console.log(`Signal blocked by filters ${strat.name}: ${direction}`);
          }
          strat.signalHistory.push(signal);
          if (strat.signalHistory.length > 50) strat.signalHistory.shift();
        }
      } else if (strat.name === 'Strategy2.0') {
        if (virtualTradeStates[strat.name].active || strat.emaFastValues.length < 2 || strat.emaSlowValues.length < 2 || strat.rsiValues.length < 2) return;
        const currentPrice = globalPrice;
        const currentEMAFast = strat.emaFastValues[strat.emaFastValues.length - 1];
        const currentEMASlow = strat.emaSlowValues[strat.emaSlowValues.length - 1];
        const currentRSI = strat.rsiValues[strat.rsiValues.length - 1];
        const lastCandle = candlePrices[candlePrices.length - 2]; // Предыдущая завершенная свеча для хвоста

        const __f2 = evaluateFilters_Strat2(currentPrice, currentEMAFast, currentEMASlow, currentRSI, lastCandle);
        filtersPass = __f2.pass;
        direction = __f2.direction;

        if (direction) {
          const signal = { timestamp: Date.now(), direction, price: currentPrice, emaFast: currentEMAFast, emaSlow: currentEMASlow, rsi: currentRSI, outcome: 'blocked' };
          console.log(`Potential signal ${strat.name}: ${direction}`);
          if (filtersPass) {
            signal.outcome = 'passed';
            console.log(`Signal passed filters ${strat.name}: ${direction}`);
            executeVirtualTrade(strat, direction); // Виртуальная
          }
          strat.signalHistory.push(signal);
          if (strat.signalHistory.length > 50) strat.signalHistory.shift();
        }
  ...
  try {
    ...
    if (tradeState.active && !tradeState.resultRecorded) {
      ...
      if (timeSinceTrade > expiryTime * 60000 * 2) { // 2x времени экспирации
        console.log("Очистка зависшего состояния сделки");
        tradeState.active = false;
      }
    }
    strategies.forEach(strat => {
      if (strat.type === 'virtual') {
        const vState = virtualTradeStates[strat.name];
        if (vState.active && !vState.resultRecorded) {
          const timeSinceTrade = currentTime - vState.startTime;
          if (timeSinceTrade > strat.expiryTime * 60000 * 2) {
            console.log(`Очистка зависшего виртуального состояния сделки для ${strat.name}`);
            vState.active = false;
          }
        }
      }
    });
    updateIndicators(); // Убедимся, что индикаторы обновляются перед UI
    checkTradingConditions();
    updateUI(currentTime);
  } catch (e) {
    console.error("Ошибка в mainLoop:", e);
  }
}
...
function addUI() {
  try {
    ...
    panel.style.cssText = ` position: fixed; bottom: 0; left: 0; z-index: 10000; width: 100%; max-height: 300px; background: rgba(30, 30, 30, 0.97); color: white; padding: 10px; font-family: Arial, sans-serif; font-size: 12px; border-top: 1px solid #444; overflow-y: auto; box-sizing: border-box; `;
    let virtualSections = '';
    strategies.forEach(strat => {
      if (strat.type === 'virtual') {
        virtualSections += `
          <div id="ui-virtual-${strat.name}">
            <h4>${strat.name} (Virtual)</h4>
            <p>Virtual Balance: <span id="ui-virtual-balance-${strat.name}">-</span></p>
            <p>Virtual Profit: <span id="ui-virtual-profit-${strat.name}">-</span></p>
            <p>Virtual Win Rate: <span id="ui-virtual-winrate-${strat.name}">0%</span></p>
          </div>
        `;
      }
    });
    panel.innerHTML = `
      <div id="bot-candle-chart">
        Bot Candles <select id="ui-tf"><option value="1m">1M</option><option value="5m">5M</option></select>
        <input type="range" id="ui-zoom" min="${CHART_ZOOM_MIN}" max="${CHART_ZOOM_MAX}" value="60">
        <input type="range" id="ui-scroll" min="0" max="0" value="0"><button id="ui-scroll-end">End</button>
        <span id="ui-chart-info">—</span>
        <div id="ui-candle-progress">Candle progress: <div id="ui-candle-progress-bar" style="width:0%;height:4px;background:#4CAF50;"></div></div>
        <canvas id="bot-candle-canvas" width="300" height="240"></canvas>
      </div>

      <h3>${appversion}</h3>
      <p>Symbol: <span id="ui-symbol">-</span></p>
      <p>Price: <span id="ui-price">0.00000</span></p>
      <p>EMA(${typeof emaPeriod!=='undefined'?emaPeriod:'-' }): <span id="ui-ema">-</span></p>
      <p>RSI(${typeof rsiPeriod!=='undefined'?rsiPeriod:'-' }): <span id="ui-rsi">-</span></p>
      <p>Start Time: <span id="ui-start-time">-</span></p>
      <p>Runtime: <span id="ui-runtime">-</span></p>
      <p>Account: <span id="ui-account-type">${mode}</span></p>
      <p>Start Balance: <span id="ui-start-balance">-</span></p>
      <p>Balance: <span id="ui-balance">-</span></p>
      <p>P&L: <span id="ui-pnl">- </span> ( <span id="ui-pnl-percent">- </span>% )</p>
      <p>Wager (Σ bets): <span id="ui-wager">0.00</span></p>
      <p>Hit Rate: <span id="ui-hit-rate">0%</span></p>
      ${virtualSections}
      <div>
        Cycle History (last 8):
        <table id="ui-cycle-history-table">
          <thead><tr><th>Cycle</th><th>Result</th><th>Amount</th></tr></thead>
          <tbody><tr><td colspan="3">No cycles yet</td></tr></tbody>
        </table>
      </div>
    `;
    ...
    if(!window.__L05_wired){
      const tfSel=document.getElementById('ui-tf');
      if(tfSel) tfSel.addEventListener('change',e=>{ chartTimeframe=e.target.value; chartOffset=0; renderCandleChart(); });
      const zoom=document.getElementById('ui-zoom');
      if(zoom) zoom.addEventListener('input',e=>{ chartBars=Math.max(CHART_ZOOM_MIN, Math.min(CHART_ZOOM_MAX, parseInt(e.target.value||60))); renderCandleChart(); });
      const scroll=document.getElementById('ui-scroll');
      if(scroll) scroll.addEventListener('input',e=>{ chartOffset=Math.max(0, parseInt(e.target.value||0)); renderCandleChart(); });
      const btn=document.getElementById('ui-scroll-end');
      if(btn) btn.addEventListener('click',()=>{ chartOffset=0; renderCandleChart(); });
...
function updateSignalHistoryUI() {
    const historyEl = document.getElementById('ui-signal-history');
    if (!historyEl || !Array.isArray(signalHistory) || signalHistory.length === 0) {
        if (historyEl) historyEl.innerHTML = 'No signals yet';
        return;
    }
    const recent = signalHistory.slice(-10).reverse(); // Newest first
    let html = '';
    recent.forEach((s, i) => {
        const signalNum = signalHistory.length - recent.length + i + 1;
        const outcomeColor = s.outcome === 'passed' ? '#4CAF50' : '#F44336';
        const filtersStr = `ATR:${s.filterResults.atr ? 'pass' : 'fail'}, STD:${s.filterResults.std ? 'pass' : 'fail'}, Dist:${s.filterResults.dist ? 'pass' : 'fail'}, Range:${s.filterResults.range ? 'pass' : 'fail'}`;
        html += `<div style="margin-bottom:2px;">Signal ${signalNum}: ${s.direction} at ${new Date(s.timestamp).toLocaleTimeString()}, Outcome: <span style="color:${outcomeColor};">${s.outcome.toUpperCase()}</span>, Filters: ${filtersStr}</div>`;
    });
    historyEl.innerHTML = html;
}
function updateUI(currentTime) {
  try {
    if (!document.getElementById('ui-symbol')) { addUI(); return; }
    ...
    safeUpdateElement('ui-account-type', mode);
    const pnl = currentBalance - startBalance;
    safeUpdateElement('ui-pnl', pnl.toFixed(2));
    const pnlPercent = startBalance > 0 ? (pnl / startBalance * 100).toFixed(2) : 0;
    safeUpdateElement('ui-pnl-percent', pnlPercent);
    safeUpdateElement('ui-start-time', startTime.toLocaleString('ru-RU', {hour: '2-digit', minute: '2-digit', second: '2-digit', day: '2-digit', month: '2-digit', year: 'numeric'}));
    const runtimeSec = Math.floor((currentTime - startTime) / 1000);
    const days = Math.floor(runtimeSec / 86400);
    const hours = Math.floor((runtimeSec % 86400) / 3600).toString().padStart(2, '0');
    const minutes = Math.floor((runtimeSec % 3600) / 60).toString().padStart(2, '0');
    const seconds = (runtimeSec % 60).toString().padStart(2, '0');
    safeUpdateElement('ui-runtime', `${days} days ${hours}:${minutes}:${seconds}`);
    // Индикаторы для Strategy1.0
    const lastEMA = strategies.find(s => s.name === 'Strategy1.0').emaValues.length > 0 ? strategies.find(s => s.name === 'Strategy1.0').emaValues[strategies.find(s => s.name === 'Strategy1.0').emaValues.length - 1] : null;
    const lastRSI = strategies.find(s => s.name === 'Strategy1.0').rsiValues.length > 0 ? strategies.find(s => s.name === 'Strategy1.0').rsiValues[strategies.find(s => s.name === 'Strategy1.0').rsiValues.length - 1] : null;
    ...
    safeUpdateElement('ui-wager', (window.totalWager || 0).toFixed(2));
    const totalTrades = tradeHistory.length;
    const wins = tradeHistory.filter(t => t.result === 'profit').length;
    const hitRate = totalTrades > 0 ? (wins / totalTrades * 100).toFixed(1) : 0;
    safeUpdateElement('ui-hit-rate', hitRate + '%');
    const tableBody = document.querySelector('#ui-cycle-history-table tbody');
    if (tableBody && Array.isArray(cycleHistory) && cycleHistory.length > 0) {
      let html = '';
      const recent = cycleHistory.slice(-8).reverse();
      recent.forEach((c, i) => {
        const cycleNum = cycleHistory.length - recent.length + i + 1;
        const resultColor = c.hitType === 'win' ? '#4CAF50' : '#F44336';
        html += `<tr><td>${cycleNum}</td><td style="color:${resultColor}">${c.hitType.toUpperCase()}</td><td>${c.pnl.toFixed(2)}</td></tr>`;
      });
      tableBody.innerHTML = html;
    } else if (tableBody) {
      tableBody.innerHTML = '<tr><td colspan="3">No cycles yet</td></tr>';
    }
    updateCycleHistoryUI();
    // Обновление виртуальных UI
    strategies.forEach(strat => {
      if (strat.type === 'virtual') {
        safeUpdateElement(`ui-virtual-balance-${strat.name}`, strat.virtualBalance.toFixed(2));
        safeUpdateElement(`ui-virtual-profit-${strat.name}`, strat.virtualProfit.toFixed(2));
        safeUpdateElement(`ui-virtual-winrate-${strat.name}`, strat.virtualWinRate.toFixed(1) + '%');
      }
    });
  } catch (e) {
...
function renderCandleChart(){
  try{
    ...
    const rect = canvas.getBoundingClientRect();
    console.log('Размеры холста: width=' + rect.width + ', height=' + rect.height);
    const dpr = window.devicePixelRatio || 1;
  ...
  }catch(e){ console.error('Ошибка renderCandleChart:', e); }
}
